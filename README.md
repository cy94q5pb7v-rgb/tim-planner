...
- Страница `/admin` для администраторов: создание пользователей и смена паролей, выдача доступа к
  Планеру, назначение администраторов и **ролей** — всё из браузера, без консоли.
...
### Веб-админка (основной способ) — `/admin`
Администратор (пользователь с `is_admin`) открывает **`/admin`** и прямо из браузера: создаёт
пользователей и меняет пароли, **выдаёт доступ к Планеру**, назначает **администраторов** и **роли**
(галочками), удаляет пользователей. Роли сразу применяются в Планере (метка человека, «Ответственные
по этапам»). Страница и API доступны только администраторам (не-админа редиректит на Планер, API → 403;
эскалация прав невозможна).

### Первый администратор (bootstrap) — CLI
Первого админа заводят скриптом (веб-админка требует уже существующего админа):

```bash
python scripts/make_user.py admin --password 'СВОЙ_ПАРОЛЬ' --admin
```
Дальше — всё через `/admin`. При желании скрипт умеет и остальное:
```bash
python scripts/make_user.py ivanov --password 'pass' --planner --roles ba,dev   # участник + роли
python scripts/make_user.py ivanov --password 'new-pass'                          # сменить пароль
```

**Права и роли:**
- `is_admin` — полный доступ (Планер + админка).
- `planner_access` — доступ к Планеру без прав администратора.
- Роли: `ba, sa, dev, test, qa, po, pm, lead` (роли `ba/sa/dev/test` подставляются в «Ответственные
  по этапам»; `po`=Product Owner, `pm`=Product Manager и прочие — организационные метки).

Без `planner_access`/`is_admin` пользователь войдёт, но увидит страницу «Нет доступа к Планеру».
...
tim-planner/
├─ backend/
│  ├─ web_app.py            # FastAPI: авторизация, вход/выход, раздача SPA + /admin, подключение API
│  ├─ admin_api.py          # админ-API: пользователи, права, роли (только is_admin)
...
│  ├─ admin.html            # страница администрирования (пользователи/права/роли)
