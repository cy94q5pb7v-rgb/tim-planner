# -*- coding: utf-8 -*-
"""ТИМ Планер — почтовые уведомления о смене статуса/этапа задачи (опционально).

planner_api.mutate() после apply_ops собирает список изменений и (если включено
через PLANNER_NOTIFY_ENABLED) зовёт notify_task_changes(changes, actor, people,
epics). Здесь в ФОНОВОМ потоке рендерится email-safe письмо и шлётся через smtplib.

Безопасность:
  - Все SMTP-настройки (host/port/user/pass) и получатель берутся из окружения
    (см. config.py / PLANNER_SMTP_*). Пароль живёт только в локальной переменной,
    НИКОГДА не логируется и не попадает в текст исключений.
  - Любой сбой SMTP гасится (try/except + logging) и не пробрасывается наружу:
    почта не должна ронять сохранение доски.
